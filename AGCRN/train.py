...
parser.add_argument('--cheb_k', default=3, type=int)
####################### model parameters #######################
...
def main():
    ...
    if args.dr:
        args.input_dim, args.horizon, args.num_nodes = training_data.in_dim-1, training_data.seq_len, training_data.num_nodes
    else:
        args.input_dim, args.horizon, args.num_nodes = training_data.in_dim, training_data.seq_len, training_data.num_nodes
    ...
    log = 'No. of features: %s, No. of nodes: %s'%(args.input_dim, args.num_nodes)
    ...
    for repeat in range(args.repeats):
        ...
        model = dynamic_reg(args, scaler).to(device)
        
        for p in model.mean_model.parameters():
            if p.dim() > 1:
