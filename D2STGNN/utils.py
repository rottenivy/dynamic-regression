...
class TrafficDataset(Dataset):
    def __init__(self, debug_mode, device, file_dir, split, transform=True, target_transform=False, scaler=None):
        ...
        if 'dr' in file_dir.split('/')[-1]:
            self.in_dim, self.seq_len, self.num_nodes = self.x.shape[-1]//2, self.x.shape[1], self.x.shape[2]
        else:
        ...
        if 'dr' in file_dir.split('/')[-1]:
            if transform:
                self.x[...,:1] = self.scaler.transform(self.x[...,:1])
                self.x[...,self.in_dim:self.in_dim+1] = self.scaler.transform(self.x[...,self.in_dim:self.in_dim+1])
            if target_transform:
                self.y[...,:1] = self.scaler.transform(self.y[...,:1])
                self.y[...,self.in_dim:self.in_dim+1] = self.scaler.transform(self.y[...,self.in_dim:self.in_dim+1])
        else:
            if transform:
                self.x[...,:1] = self.scaler.transform(self.x[...,:1])
            if target_transform:
