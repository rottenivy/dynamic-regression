...
parser.add_argument('--covariates',type=str,default='spd_tod_dow',help='data path')
...
# args.epochs = 3
# args.data = "pemsd7m"
# args.covariates = "spd_tod_dow_dr12"
...
def main():
    ...
    for repeat in range(args.repeats):
        ...
        if args.loss_type == 'full':
            ...
            grouped_parameters = [
                {"params": cov_fac_params, 'lr': args.lrate, 'weight_decay': args.wdecay*100},  # TODO add mixture projecter to this
                {"params": others, 'lr': args.lrate, 'weight_decay': args.wdecay}]

            optimizer = torch.optim.Adam(grouped_parameters, lr=args.lrate, weight_decay=args.wdecay)
        else:
        ...
        for e in range(args.epochs):
            t1 = time.time()
            # mtrain_loss, mtrain_mape, mtrain_rmse = train_epoch(dataloader=train_dataloader, model=model, loss_fn=loss_fn, optimizer=optimizer, scheduler_L=scheduler, out_dim=args.out_dim, scaler=scaler, rho=args.rho, clip=clip)
    ...
    # y_test_hat = test_epoch(test_dataloader, model, n_sample=100, scaler=scaler)  # B, n_sample, Q, N
