...
parser.add_argument('--data',type=str,default='pems08_flow',help='data path')
parser.add_argument('--covariates',type=str,default='flow_tod',help='data path')
####################### model parameters #######################
...
def main():
    ...
    for repeat in range(args.repeats):
        ...
        rrmse_val = utils.masked_rrmse(test_epoch(val_dataloader, model, scaler=scaler), val_data.y[...,0].to(device)).item()
        rrmse_val_all.append(rrmse_val)

        y_test_hat = test_epoch(test_dataloader, model, n_sample=80, scaler=scaler)
    ...
    log = 'avg val rrmse: {:.4f}'
    utils.log_string(log_file, log.format(np.mean(np.array(rrmse_val_all))))
