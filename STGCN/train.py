...
def main():
    ...
    val_data = utils.TrafficDataset(debug_mode, device=device, file_dir=os.path.join(args.data_path, args.data, args.covariates), split='val', scaler=args.scaler)
    testing_data = utils.TrafficDataset(debug_mode, device=device, file_dir=os.path.join(args.data_path, args.data, args.covariates), split='test', scaler=args.scaler)
    ...
    for repeat in range(args.repeats):
        ...
        for e in range(args.epochs):
            t1 = time.time()
            mtrain_loss, mtrain_mape, mtrain_rmse = train_epoch(dataloader=train_dataloader, model=model, loss_fn=loss_fn, optimizer=optimizer, out_dim=args.out_dim, scaler=args.scaler, rho=args.rho, clip=clip)
            t2 = time.time()

            s1 = time.time()
            mvalid_loss, mvalid_mape, mvalid_rmse = eval_epoch(dataloader=val_dataloader, model=model, loss_fn=loss_fn, out_dim=args.out_dim, scaler=args.scaler, rho=args.rho)
            s2 = time.time()
            ...
            writer.add_scalars('Loss/train', {'train_loss':mtrain_loss, 'val_loss':mvalid_rmse}, e)

            if early_stop:
                break

            # if early_stop:
            #     if args.loss_type == 'full' and not model.cov_model.L_n.requires_grad:
            #         model.load_state_dict(torch.load(os.path.join(args.save, note, 'repeat%s'%(repeat+1), 'best_model.pth'))['model_state_dict'])
            #         model.cov_model.L_n.requires_grad = True
            #         model.cov_model.L_q.requires_grad = True
        ...
        y_test_hat = test_epoch(test_dataloader, model, scaler=args.scaler)
            
        utils.log_string(log_file, 'Performance in each prediction step:')
        utils.log_string(log_file, '\t\tMAE\t\tRMSE\t\tMAPE\t\tMSE\t\tRRMSE')
    ...
    log = 'The average best val loss is %.4f'%(np.mean(best_val_loss))
    utils.log_string(log_file, log)
    ...
    #     model.load_state_dict(torch.load(os.path.join(args.save, note, 'repeat%s'%(repeat+1), 'best_model.pth'), map_location=args.device)['model_state_dict'])
    #     y_test_hat = test_epoch(test_dataloader, model, n_sample=100, scaler=args.scaler)  # B, n_sample, Q, N
