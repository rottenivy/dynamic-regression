...
def train_epoch(dataloader, model, loss_fn, optimizer, out_dim, rho=0.0, scheduler_L=None, scaler=None, clip=None):
    ...
    for batch, (x, y) in enumerate(dataloader):
        ...
        if clip is not None:
            #1 clip all model parameters
            # torch.nn.utils.clip_grad_norm_(model.parameters(), clip)
            #2 clip parameters only in the cov model
            torch.nn.utils.clip_grad_norm_(model.cov_model.parameters(), clip)
            #3 clip only the low rank parameters
            # torch.nn.utils.clip_grad_norm_([model.cov_model.L_n, model.cov_model.L_q], clip)
